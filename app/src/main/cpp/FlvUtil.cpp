...
void flushBuff(flv_t *a) {
    la_flush(a);
}

static size_t la_fwrite(const void *buf, size_t size, size_t count, flv_t *a) {
    //write to buff, if buff is full , flush it
    long length = static_cast<long>(size * count);
//    if (length > BUFFMAX) {// if length > buff max length, write it to file directly.
//        la_flush(a);
...
//    } else {
    memcpy(a->buff + a->buff_index, buf, static_cast<size_t>(length));
    a->buff_index += length;
//    }
