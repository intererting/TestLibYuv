...
Java_com_example_android_camera2basic_YuvUtil_compressYUV(JNIEnv *env, jclass type,
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_android_camera2basic_YuvUtil_rotateYUV(JNIEnv *env, jclass type, jbyteArray src_, jint strideY,
                                                        jbyteArray srcUV_, jint strideUV, jbyteArray dstY_,
                                                        jint dstStrideY, jbyteArray dstU_, jint dstStrideU,
                                                        jbyteArray dstV_, jint dstStrideV, jint width, jint height,
                                                        jint rotate) {
    jbyte *src = env->GetByteArrayElements(src_, NULL);
    jbyte *srcUV = env->GetByteArrayElements(srcUV_, NULL);
    jbyte *dstY = env->GetByteArrayElements(dstY_, NULL);
    jbyte *dstU = env->GetByteArrayElements(dstU_, NULL);
    jbyte *dstV = env->GetByteArrayElements(dstV_, NULL);

    libyuv::NV12ToI420Rotate(reinterpret_cast<const uint8 *>(src), strideY,
                             reinterpret_cast<const uint8 *>(srcUV), strideUV,
                             reinterpret_cast<uint8 *>(dstY), dstStrideY,
                             reinterpret_cast<uint8 *>(dstU), dstStrideU,
                             reinterpret_cast<uint8 *>(dstV), dstStrideV, width,
                             height, static_cast<libyuv::RotationMode>(rotate));

    env->ReleaseByteArrayElements(src_, src, 0);
    env->ReleaseByteArrayElements(srcUV_, srcUV, 0);
    env->ReleaseByteArrayElements(dstY_, dstY, 0);
    env->ReleaseByteArrayElements(dstU_, dstU, 0);
    env->ReleaseByteArrayElements(dstV_, dstV, 0);
}